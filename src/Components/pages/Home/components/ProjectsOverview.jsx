...
function ProjectsOverview() {
    ...
    return (
        <section
            id="projects"
            className="py-16 text-white group transition-colors duration-500"
        >
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center gap-12">
                ...
                <div className="flex-1 relative w-full h-80 md:h-96 overflow-hidden rounded-lg shadow-lg">{images.map((src, index) => (
                    <div
                        key={index}
                        className={`absolute inset-0 w-full h-full rounded-2xl overflow-hidden border-3 border-orange-500 
                        transition-all duration-700 group-hover:border-transparent
